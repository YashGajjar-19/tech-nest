  ...
  return (
    <section className="px-6 max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-12 mt-12 animate-in fade-in duration-500">
      ...
      <div className="col-span-1 md:col-span-2 flex flex-col justify-center space-y-6 bg-surface border border-border-subtle rounded-3xl p-8 lg:p-10 shadow-sm">
        ...
        <ScoreBar label="Display Quality" score={Math.round((metrics.Display || metrics.display || 94) * 10)} size="lg" />
        <ScoreBar label="Processing Power" score={Math.round((metrics.Performance || metrics.performance || 98) * 10)} size="lg" />
        <ScoreBar label="Camera System" score={Math.round((metrics.Camera || metrics.camera || 89) * 10)} size="lg" />
        <ScoreBar label="Battery Life" score={Math.round((metrics.Battery || metrics.battery || 84) * 10)} size="lg" />
        <ScoreBar label="Design & Build" score={Math.round((metrics.Design || metrics.design || 92) * 10)} size="lg" />
      </div>
