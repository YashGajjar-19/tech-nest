    ...
    <Card className="border-border-subtle bg-surface shadow-none overflow-hidden p-0">
      <CardHeader className="flex flex-row items-center justify-between pb-6 pt-6 px-6">
        <div className="space-y-1.5 focus:outline-none">
          ...
          <CardDescription>
            Active comparisons and intelligence tracking.
          </CardDescription>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-0">
        <div className="divide-y divide-border-subtle">
          {MOCK_PIPELINE.map((decision) => (
            <div
              key={decision.id}
              className="p-6 hover:bg-bg-primary/50 transition-colors"
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6">
                <div className="flex flex-col bg-bg-primary rounded-xl px-4 py-3 border border-border-subtle w-full max-w-sm">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-medium text-text-secondary uppercase tracking-wider flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-brand" /> AI Lean
                    </span>
                    <span className="text-xs text-brand font-semibold text-right">
                ...
                <Button variant="ghost" className="shrink-0 group">
                  <a href={`/compare`}>
                    Resume Decision{" "}
