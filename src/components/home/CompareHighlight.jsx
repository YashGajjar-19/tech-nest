...
export default function CompareHighlight ( { products = [] } )
{

    // Abstracting out the mock data for the visual demo
    // If we have actual products, pick the first two (or top two brands)
    const rawLeft = products.find( p => p.brands?.name === "Apple" ) || products[ 0 ];
    const rawRight = products.find( p => p.brands?.name === "Samsung" ) || products[ 1 ];

    const leftDevice = rawLeft ? {
        brand: rawLeft.brands?.name || "Brand A",
        name: rawLeft.model_name || "Device 1",
        image: rawLeft.image_url,
    } : { brand: "Apple", name: "iPhone 16 Pro", image: null };

    const rightDevice = rawRight ? {
        brand: rawRight.brands?.name || "Brand B",
        name: rawRight.model_name || "Device 2",
        image: rawRight.image_url,
    } : { brand: "Samsung", name: "Galaxy S24 Ultra", image: null };
                    ...
                    >
                        ...
                        <div className="rounded-[2.5rem] border border-border-color bg-bg-card/60 backdrop-blur-2xl shadow-premium-xl overflow-hidden">
                            ...
                            <div className="grid grid-cols-2 p-6 md:p-8 border-b border-border-color/50 relative">
                                ...
                                >
                                    <div className="w-20 h-24 mb-4 relative drop-shadow-xl flex items-center justify-center">
                                        { leftDevice.image ? (
                                            <img src={ leftDevice.image } alt={ leftDevice.name } className="h-full object-contain" />
                                        ) : (
                                            <>
                                    ...
                                    <p className="text-[10px] font-bold tracking-widest text-text-secondary uppercase mb-1">{ leftDevice.brand }</p>
                                    <h4 className="text-base font-semibold text-text-primary">{ leftDevice.name }</h4>
                                </motion.div>
                                ...
                                >
                                    <div className="w-20 h-24 mb-4 relative drop-shadow-xl flex items-center justify-center">
                                        { rightDevice.image ? (
                                            <img src={ rightDevice.image } alt={ rightDevice.name } className="h-full object-contain" />
                                        ) : (
                                            <>
                                                <div className="absolute inset-0 border-2 border-text-primary/20 rounded-2xl bg-linear-to-br from-bg-main to-bg-card" />
                                    ...
                                    <p className="text-[10px] font-bold tracking-widest text-text-secondary uppercase mb-1">{ rightDevice.brand }</p>
                                    <h4 className="text-base font-semibold text-text-primary">{ rightDevice.name }</h4>
                                </motion.div>
                            </div>
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-text-primary/80 leading-relaxed">
                                        The <span className="font-medium">{ leftDevice.name }</span> edges out in raw computational tasks, but the <span className="font-medium">{ rightDevice.name }</span> remains unmatched for camera versatility and productivity.
                                    </p>
                                </div>
                            </motion.div>
