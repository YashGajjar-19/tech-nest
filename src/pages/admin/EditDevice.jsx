...
import { getBrands, getDeviceById, updateFullDevice } from "@/services/apiDevices";
...
export default function EditDevice ()
{
    ...
    useEffect( () =>
    {
        async function loadData ()
        {
            ...
            {
                console.error( error );
                toast.error( "Failed to recover device data" );
    ...
    if ( loading ) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
            <div className="animate-spin w-10 h-10 border-4 border-text-primary/10 border-t-hyper-cyan rounded-full mb-4"></div>
            <p className="text-sm font-semibold text-text-secondary italic">Retrieving device blueprint...</p>
        </div>
    );
            ...
            <header className="mb-10 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm font-medium text-text-secondary mt-1">
                        Editing: <span className="text-text-primary font-bold">{ formData.model_name }</span>
                    </p>
                </div>
            </header>
